...
import {
  ...
  createDispatcher as createDispatcherCore,
  invalidateResult,
} from "./dispatch.mjs";
...
const testEventLogs = new Map();

// Tests inject one writer per disposable state directory. Production owns this
// wiring in createServer; keeping it explicit here prevents the dispatcher from
// masking writer-identity bugs with a lazy second event log.
function createDispatcher(options) {
  let eventLog = options.eventLog;
  if (!eventLog) {
    eventLog = testEventLogs.get(options.stateDir);
    if (!eventLog) {
      eventLog = createEventLog({ stateDir: options.stateDir });
      testEventLogs.set(options.stateDir, eventLog);
    }
  }
  return createDispatcherCore({ ...options, eventLog });
}
...
function isAtomicWriteFor(path, name) {
  const file = basename(String(path));
  return file === name || (file.startsWith(`.${name}.`) && file.endsWith(".tmp"));
}
...
function profileFixture(setup, {
  ...
  return {
    ...
    profile: createExecutionProfile({
      agentLane: lane,
      command: lane === "codex" ? process.execPath : "claude",
      ...(lane === "codex" ? {
        executableResolvedPath: process.execPath,
        binaryPinned: false,
      } : {}),
      companionPath,
      companionDigest: companionPath && existsSync(companionPath)
        ? createHash("sha256").update(readFileSync(companionPath)).digest("hex")
        : null,
      env,
...
async function fixture(t, projectOverrides = {}, defaults = {}) {
  ...
  _setCodexAppServerProber(async () => ({
    userAgent: "fixture-codex/1.0",
    platformFamily: "unix",
...
function useLegacyCompanion(setup, resolver = () => setup.companionPath) {
  setup.project.legacyCodexCompanion = true;
  _setCompanionResolver(resolver);
}
...
function stubConvoyRuntime(
  ...
  issues,
  calls = [],
  { changed = PROBE_CHANGED_FILE, readyIssues = issues, gitHandler } = {},
) {
  ...
  _setRunFile(withDispatchChanges(async (file, args, options = {}) => {
    ...
    assert.equal(file, "git");
    if (gitHandler) {
      const handled = await gitHandler(args, options);
...
async function waitForState(dispatcher, id, states) {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    const remove = dispatcher.onEvent((event) => {
      ...
      if (
        ["prepare_failed", "failed", "stopped", "completed_empty", "needs_input"].includes(record?.state) &&
        !wanted.has(record.state)
      ) {
        remove();
        rejectPromise(new Error(
          `dispatch ${id} reached ${record.state} while waiting for ${[...wanted].join("|")}: ${record.exitSummary}`,
        ));
        return;
      }
...
async function seedDispatch(setup, overrides = {}) {
  ...
  if (record.lane === "codex" && !Object.hasOwn(overrides, "codexAdapter")) {
    record.codexAdapter = setup.project.legacyCodexCompanion
      ? "legacy-companion"
      : "app-server";
  }
...
async function writeWorkspaceToken(record, {
  path = record.worktreePath,
  kind = "dispatch",
} = {}) {
  await writeFile(
    join(path, ".atelier-workspace.json"),
    `${JSON.stringify({
      recordId: record.id,
...
function stubFastForwardMerge(setup, { branchHead = "validated-head", mainHead = "main-head" } = {}) {
  _setRunFile(async (file, args) => {
    assert.equal(file, "git");
    if (args[2] === "rev-parse" && args[3] === "--verify") return `${branchHead}\n`;
    if (args[2] === "rev-parse" && args[3] === "main") return `${mainHead}\n`;
    if (args[2] === "merge-base") return "";
    return "";
  });
}
...
test("boot compaction counts malformed lines before dropping their evidence", async (t) => {
  const setup = await fixture(t);
  const dispatchDir = join(setup.state, "dispatches");
  const indexPath = join(dispatchDir, "index.jsonl");
  await mkdir(dispatchDir, { recursive: true });
  ...
  await writeFile(
    indexPath,
    `${Array.from({ length: 1_001 }, () => record).join("\n")}\n{"broken":\n`,
  );
  const warnings = [];
  _setPersistenceLogger({ error: (message) => warnings.push(message) });

  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  assert.equal(dispatcher.get("compaction-seed").state, "failed");
  assert.ok(warnings.some((message) => /skipped 1 malformed index line/.test(message)));
  assert.equal((await readFile(indexPath, "utf8")).includes('{"broken":'), false);
});
...
for (const stateFile of ["dispatches/index.jsonl", "queue.json", "convoys.json"]) {
  test(`dispatcher boot refuses a symlinked ${stateFile} state file`, async (t) => {
    const setup = await fixture(t);
    const statePath = join(setup.state, ...stateFile.split("/"));
    const target = join(setup.root, `outside-${basename(stateFile)}`);
    await mkdir(dirname(statePath), { recursive: true });
    await writeFile(target, stateFile.endsWith("index.jsonl") ? "" : "{}\n");
...
test("execution profile resolution ignores relative and empty PATH components", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "atelier-relative-path-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const bin = join(root, "bin");
  await mkdir(bin);
  const command = process.platform === "win32" ? "relative-provider.cmd" : "relative-provider";
  ...
  const relativeBin = relative(process.cwd(), bin);
  if (isAbsolute(relativeBin)) {
    t.skip("temporary directory is on a different Windows drive");
    return;
  }

  assert.equal(
    resolveExecutable(command, { PATH: `${relativeBin}${delimiter}${delimiter}` }),
    null,
  );
  assert.equal(resolveExecutable(command, { PATH: bin }), executable);
});
...
test("dispatch runs queued -> preparing -> running -> completed with prompt posture", async (t) => {
  ...
  assert.equal(launches[0].command, record.executionProfile.executable.resolvedPath);
  ...
  const workspaceIdentity = JSON.parse(await readFile(
    join(record.worktreePath, ".atelier-workspace.json"),
    "utf8",
  ));
  assert.deepEqual(
    { recordId: workspaceIdentity.recordId, project: workspaceIdentity.project, kind: workspaceIdentity.kind },
    { recordId: id, project: "fixture", kind: "dispatch" },
  );
  assert.equal((await stat(join(record.worktreePath, ".atelier-workspace.json"))).mode & 0o777, 0o600);
  assert.deepEqual(rawRecord(setup, record.id).executionProfile, record.executionProfile);
  const originalDigest = record.executionProfile.envDigest;
  record.executionProfile.envDigest = "consumer-mutation";
  assert.equal(dispatcher.get(id).executionProfile.envDigest, originalDigest);
  assert.equal(rawRecord(setup, id).executionProfile.envDigest, originalDigest);
  assert.deepEqual(
...
test("workspace token naming collision hard-fails preparation and removes the new checkout", async (t) => {
  ...
  _setRunFile(withDispatchChanges(async (file, args) => {
    ...
    if (args[2] === "worktree" && args[3] === "add" && args[4] === "-b") {
      createdPath = args[6];
      await mkdir(createdPath, { recursive: true });
      await writeFile(join(createdPath, ".atelier-workspace.json"), "tracked project bytes\n");
      return "";
    }
    if (args[2] === "worktree" && args[3] === "remove") {
      await rm(args[4], { recursive: true, force: true });
  ...
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const { id } = await dispatcher.dispatch({ project: "fixture", prompt: "collision" });
  const failed = await waitForState(dispatcher, id, ["prepare_failed"]);
  assert.match(failed.exitSummary, /workspace identity naming collision/);
...
test("execution-profile acceptance refuses when its re-stamp is not durable", async (t) => {
  const setup = await fixture(t, { dispatchEnv: { PLAN_PROFILE_FLAG: "before" } });
  stubPreparation();
  let launches = 0;
  _setSpawner(() => {
    launches += 1;
    return claudeResultChild({ summary: "1. Implement it." });
  });
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const { id } = await dispatcher.dispatch({
    project: "fixture",
    prompt: "persist accepted profile",
    planFirst: true,
  });
  await waitForState(dispatcher, id, ["plan_ready"]);
  setup.project.dispatchEnv = { PLAN_PROFILE_FLAG: "after" };
  _setPersistenceFileOps({
    appendDescriptorSync(descriptor, contents, options, path) {
      if (path.endsWith("index.jsonl")) {
        ...
        if (record.executionProfile?.superseded?.length > 0) {
          throw Object.assign(new Error("fixture acceptance EIO"), { code: "EIO" });
        }
      }
  ...
  await assert.rejects(
    ...
    (error) => error.status === 409 && /acceptance refused: persistence degradation/.test(error.message),
  );
  assert.equal(launches, 1);
  assert.equal(dispatcher.get(id).executionProfile.superseded, undefined);
  assert.equal(dispatcher.get(id).state, "plan_ready");
});
...
test("a refused resume creates no legacy or accepted execution-profile stamp", async (t) => {
  const setup = await fixture(t, {}, { concurrentDispatchCap: 0 });
  const seeded = await seedDispatch(setup, {
    id: "profile-refused-before-admission",
    sessionId: "legacy-session",
  ...
  await assert.rejects(
    dispatcher.reply(seeded.id, {
      text: "do not stamp this",
      acceptExecutionProfile: true,
      actor: "human",
    }),
    /Concurrent dispatch cap exceeded \(0\)/,
  );
  assert.equal(dispatcher.get(seeded.id).executionProfile, null);
  assert.equal(rawRecord(setup, seeded.id).executionProfile ?? null, null);
});
...
test("running reply writes the accepted Claude JSONL shape to piped stdin", async (t) => {
  ...
  await waitForState(dispatcher, id, ["running"]);
  assert.equal(launch.command, dispatcher.get(id).executionProfile.executable.resolvedPath);
...
test("persistence write failures warn, keep transitions live, and resync on recovery", async (t) => {
  ...
  let writesFail = true;
  _setPersistenceFileOps({
    appendDescriptorSync(descriptor, contents, options) {
      if (writesFail) {
  ...
  assert.equal(await readFile(join(setup.state, "dispatches", "index.jsonl"), "utf8"), "");
  assert.equal(await readFile(join(setup.state, "dispatches", `${id}.jsonl`), "utf8"), "");
  ...
  const persistedEvents = (await readFile(
    ...
    .split("\n")
    .map(JSON.parse);
  assert.equal(persistedEvents[0].seq, 1);
  assert.equal(persistedEvents.at(-1).state, "completed");
  assert.equal((await stat(join(setup.state, "dispatches", "index.jsonl"))).mode & 0o777, 0o600);
  assert.equal((await stat(join(setup.state, "dispatches", `${id}.jsonl`))).mode & 0o777, 0o600);
});
...
test("an event append whose fsync fails is not duplicated on retry", async (t) => {
  const setup = await fixture(t);
  stubPreparation();
  _setSpawner(() => successfulChild());
  let appendedPath;
  let failedEventFsync = false;
  _setPersistenceFileOps({
    appendDescriptorSync(descriptor, contents, options, path) {
      appendFileSync(descriptor, contents, options);
      appendedPath = path;
    ...
    fsyncSync(descriptor) {
      if (!failedEventFsync && appendedPath?.endsWith(".jsonl") &&
        !appendedPath.endsWith("index.jsonl")) {
        failedEventFsync = true;
        appendedPath = undefined;
        throw Object.assign(new Error("fixture event fsync failure"), { code: "EIO" });
      }
      appendedPath = undefined;
      fsyncSync(descriptor);
    },
  ...
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const { id } = await dispatcher.dispatch({ project: "fixture", prompt: "fsync once" });
  await waitForState(dispatcher, id, ["completed"]);
  assert.equal(failedEventFsync, true);
  ...
  _setPersistenceFileOps({
    appendDescriptorSync(descriptor, contents, encoding, path) {
      if (!wrotePartialRecord && path.endsWith("index.jsonl")) {
        ...
        appendFileSync(descriptor, contents.slice(0, 12), encoding);
...
test("dispatcher boot skips and repairs a malformed index tail before the next append", async (t) => {
  ...
  assert.ok(logLines.some((line) => line.includes("skipped 1 malformed index line")));
...
test("persistence degradation blocks forced merge and queue automation while manual dispatch continues", async (t) => {
  ...
  stubPreparation({
    onCommand(file, args) {
      if (file === fakeBr && args[0] === "ready") readyCalls += 1;
    },
  });
  _setSpawner(() => successfulChild());
  _setPersistenceFileOps({
    writeDescriptorSync(descriptor, contents, options, path) {
      if (queueWritesFail && isAtomicWriteFor(path, "queue.json")) {
        throw Object.assign(new Error("fixture queue disk outage"), { code: "EIO" });
      }
  ...
  const dispatcher = createDispatcher({
    registry: setup.registry,
    stateDir: setup.state,
    eventLog: { append: (kind, payload) => logged.push({ kind, payload }) },
  });

  const queue = dispatcher.setQueue("fixture", { enabled: true });
  assert.equal(queue.persistenceDegraded, true);
  assert.ok(queue.persistenceFailureTargets.some((path) => path.endsWith("queue.json")));
  await dispatcher.drainQueuesOnce();
  assert.equal(readyCalls, 0);
  assert.ok(logged.some(({ kind, payload }) =>
    kind === "queue.drain" &&
    payload.decision === "skipped" &&
    payload.reason === "persistence-degraded"));

  const manual = await dispatcher.dispatch({ project: "fixture", prompt: "manual recovery work" });
  const completed = await waitForState(dispatcher, manual.id, ["completed"]);
  ...
  assert.equal((await stat(join(setup.state, "queue.json"))).mode & 0o777, 0o600);
...
test("merge retries and clears a transient persistence failure without restart", async (t) => {
  ...
  _setPersistenceFileOps({
    writeDescriptorSync(descriptor, contents, options, path) {
      if (queueWritesFail && isAtomicWriteFor(path, "queue.json")) {
        throw Object.assign(new Error("fixture transient queue failure"), { code: "EIO" });
      }
      writeFileSync(descriptor, contents, options);
    },
  });
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  assert.equal(dispatcher.setQueue("fixture", { enabled: true }).persistenceDegraded, true);
  ...
  assert.equal(merged.merged.strategy, "ff");
  assert.equal(merged.persistenceDegraded, false);
  assert.equal(JSON.parse(await readFile(join(setup.state, "queue.json"), "utf8")).fixture.enabled, true);
});

test("merge refuses persistence degradation that lands during late git probes", async (t) => {
  const setup = await fixture(t, { tracker: "committed" });
  const seeded = await seedDispatch(setup);
  let dispatcher;
  ...
  _setPersistenceFileOps({
    writeDescriptorSync(descriptor, contents, options, path) {
      if (failQueueWrite && isAtomicWriteFor(path, "queue.json")) {
        throw Object.assign(new Error("fixture late merge EIO"), { code: "EIO" });
      }
      writeFileSync(descriptor, contents, options);
    },
  });
  ...
  await assert.rejects(
    ...
    (error) => error.status === 409 && /persistence degradation gate failed/.test(error.message),
  );
  assert.equal(fetches, 0);
  assert.equal(dispatcher.get(seeded.id).merged, null);
});

test("queue drain rechecks persistence immediately before unattended dispatch", async (t) => {
  const setup = await fixture(t, { tracker: "committed" });
  ...
  _setPersistenceFileOps({
    writeDescriptorSync(descriptor, contents, options, path) {
      if (failQueueWrite && isAtomicWriteFor(path, "queue.json")) {
        throw Object.assign(new Error("fixture late drain EIO"), { code: "EIO" });
      }
      writeFileSync(descriptor, contents, options);
    },
  });
  _setPersistenceLogger({ error() {} });
  _setRunFile(async (file, args) => {
    if (file === fakeBr && args[0] === "ready") {
      failQueueWrite = true;
      dispatcher.setQueue("fixture", { enabled: true });
      return `${JSON.stringify([{ id: "fixture-late", priority: 1 }])}\n`;
    }
    if (file === "git" && args[2] === "worktree" && args[3] === "add") {
      worktreeAdds += 1;
    }
...
test("an unpersisted verification verdict blocks merge until its record lands", async (t) => {
  const setup = await fixture(t, { verifyCommands: ["node --test fixture.test.mjs"] });
  stubPreparation();
  _setSpawner(() => successfulChild());
  let verdictWritesFail = true;
  _setPersistenceFileOps({
    appendDescriptorSync(descriptor, contents, options, path) {
      if (path.endsWith("index.jsonl")) {
        const record = JSON.parse(contents.trim());
        if (verdictWritesFail && record.verify?.state === "passed") {
          throw Object.assign(new Error("fixture verdict write failed"), { code: "EIO" });
        }
      }
      appendFileSync(descriptor, contents, options);
    },
  ...
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const started = await dispatcher.dispatch({ project: "fixture", prompt: "verify durably" });
  const completed = await waitForState(dispatcher, started.id, ["completed"]);
  assert.equal(completed.verify.state, "passed");
  assert.equal(completed.persistenceDegraded, true);
  ...
  verdictWritesFail = false;
  setup.project.verifyCommands = [];
  stubFastForwardMerge(setup, { branchHead: FIXTURE_BASE_COMMIT });
  const merged = await dispatcher.merge(started.id);
  assert.equal(merged.merged.strategy, "ff");
  assert.equal(merged.persistenceDegraded, false);
  assert.equal(rawRecord(setup, started.id).verify.state, "passed");
});

test("corrupt queue state is preserved and merge retry durably acknowledges it", async (t) => {
  const setup = await fixture(t, { tracker: "committed" });
  const seeded = await seedDispatch(setup);
  const corrupt = Buffer.from("{ definitely not queue json\n", "utf8");
  await writeFile(join(setup.state, "queue.json"), corrupt);
  const logLines = [];
  _setPersistenceLogger({ error: (line) => logLines.push(line) });
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });

  const evidenceName = (await readdir(setup.state)).find((name) =>
    name.startsWith("queue.json.corrupt-"));
  assert.ok(evidenceName);
  assert.deepEqual(await readFile(join(setup.state, evidenceName)), corrupt);
  assert.equal((await stat(join(setup.state, evidenceName))).mode & 0o777, 0o600);
  assert.ok(logLines.some((line) => /corrupt state.*queue\.json/.test(line)));
  assert.equal(dispatcher.getQueue("fixture").persistenceDegraded, true);
  stubFastForwardMerge(setup);
  const merged = await dispatcher.merge(seeded.id);
  assert.equal(merged.merged.strategy, "ff");
  assert.equal(merged.persistenceDegraded, false);
  ...
  assert.ok(existsSync(join(setup.state, evidenceName)));
});

test("trackerless registry retries corrupt queue state to a durable empty snapshot", async (t) => {
  const setup = await fixture(t);
  await mkdir(setup.state, { recursive: true });
  await writeFile(join(setup.state, "queue.json"), "{ broken queue\n");
  _setPersistenceLogger({ error() {} });
  const dispatcher = createDispatcher({
  ...
  await dispatcher.drainQueuesOnce();
  assert.deepEqual(JSON.parse(await readFile(join(setup.state, "queue.json"), "utf8")), {});
  assert.ok((await readdir(setup.state)).some((name) => name.startsWith("queue.json.corrupt-")));
});

test("schema-invalid queue entries are preserved and degrade instead of filtering silently", async (t) => {
  const setup = await fixture(t, { tracker: "committed" });
  await mkdir(setup.state, { recursive: true });
  const raw = `${JSON.stringify({
    fixture: {
      enabled: true,
      ticketAttempts: { broken: { attempts: 0 } },
    },
  })}\n`;
  await writeFile(join(setup.state, "queue.json"), raw);
  _setPersistenceLogger({ error() {} });

  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  assert.equal(dispatcher.getQueue("fixture").persistenceDegraded, true);
  const evidence = (await readdir(setup.state)).find((name) =>
    name.startsWith("queue.json.corrupt-"));
  assert.ok(evidence);
  assert.equal(await readFile(join(setup.state, evidence), "utf8"), raw);
});

test("schema-invalid convoy entries are preserved and degrade instead of filtering silently", async (t) => {
  const setup = await fixture(t);
  await mkdir(setup.state, { recursive: true });
  const raw = `${JSON.stringify([{ id: "broken", project: "fixture" }])}\n`;
  await writeFile(join(setup.state, "convoys.json"), raw);
  _setPersistenceLogger({ error() {} });

  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const queue = dispatcher.getQueue("fixture");
  assert.equal(queue.persistenceDegraded, true);
  assert.ok(queue.persistenceFailureTargets.some((path) => path.endsWith("convoys.json")));
  const evidence = (await readdir(setup.state)).find((name) =>
...
test("clean detached verification persists an immutable attestation and removes its worktree", async (t) => {
  ...
  stubPreparation({
    onCommand: (_file, args) => {
      gitCalls.push([...args]);
      if (args[2] === "worktree" && args[3] === "remove" && args[4].includes("verify-worktrees")) {
        verifyIdentity = JSON.parse(readFileSync(
          join(args[4], ".atelier-workspace.json"),
          "utf8",
        ));
      }
    },
  ...
  assert.deepEqual(
    { recordId: verifyIdentity.recordId, project: verifyIdentity.project, kind: verifyIdentity.kind },
    { recordId: id, project: "fixture", kind: "verify" },
  );
...
test("verification removes a tokenless partial checkout left by a timed-out worktree add", async (t) => {
  ...
  assert.equal(calls.some(({ args }) =>
    args[2] === "worktree" && args[3] === "remove" && args[4] === verificationWorktree), true);
...
test("new Codex records default to app-server and accept a content-digest re-pin", async (t) => {
  ...
  _setSpawner((command, args, options) => {
    launches.push({ command, args, options });
    return codexLaunchChild(args[args.indexOf("--job-id") + 1]);
  });
  _setRunFile(async (file, args) => {
    if (isOutcomeDiffProbe(args)) return PROBE_CHANGED_FILE;
    if (file === "git" && args[2] === "worktree") {
...
test("app-server records retain their fence across dispatcher reattach and unproven stop", async (t) => {
  ...
  await worker.ready;
  const workerIdentity = processStartIdentity(worker.child.pid);
  _setProbe(async () => ({ git: { dirtyCount: 0, branch: "main" } }));
  _setGitDirFileOps({ writeFileSync() {}, unlinkSync() {} });
  ...
  _setRunFile(async (file, args) => {
    ...
    if (isCommand(file, "node") && args[1] === "status") {
      return JSON.stringify({
        ...
        pidStartIdentity: workerIdentity,
        threadId: "app-server-restart-thread",
      });
    }
    if (isCommand(file, "node") && args[1] === "cancel") {
      cancelCalls += 1;
      return JSON.stringify({ finish: false, warning: "fixture worker exit is unproven" });
    }
    if (file === "git" && ["status", "log"].includes(args[2])) return "";
    throw new Error(`unexpected command: ${file} ${args.join(" ")}`);
  });
  ...
  const { id } = await first.dispatch({
    project: "fixture",
    prompt: "survive an Atelier restart",
    lane: "codex",
    verify: false,
  });
  await waitForConditionOverTime(
    () => rawRecord(setup, id).codexWorkerPid === worker.child.pid,
    "app-server live polling did not persist its worker fence",
  );
  ...
  const stopped = await restarted.stop(id);
  assert.equal(cancelCalls, 1);
  assert.equal(stopped.state, "stopping");
  assert.equal(rawRecord(setup, id).codexWorkerPid, worker.child.pid);
  assert.ok(stopped.warnings.includes("fixture worker exit is unproven"));
  assert.equal(worker.child.exitCode, null);
  await restarted.shutdown({ graceMs: 0 });
});

test("a refused running Codex reply preserves the adapter-owned durable environment", async (t) => {
  const setup = await fixture(t, { legacyCodexCompanion: true });
  _setProbe(async () => ({ git: { dirtyCount: 0, branch: "main" } }));
  useLegacyCompanion(setup);
  _setGitDirFileOps({ writeFileSync() {}, unlinkSync() {} });
  ...
  _setSpawner((command, args, options) => {
    launches.push({ command, args, options });
    return codexLaunchChild("codex-running-profile");
  });
  _setRunFile(async (file, args, options = {}) => {
    if (file === "git" && args[2] === "worktree") {
      await mkdir(args[6], { recursive: true });
    ...
    if (isCommand(file, "node") && ["status", "cancel"].includes(args[1])) {
      companionCommands.push({ operation: args[1], options });
      return args[1] === "status"
        ? JSON.stringify({ status: "running", job: { status: "running" } })
        : "{}";
    }
    if (file === "git" && ["status", "log"].includes(args[2])) return "";
    throw new Error(`unexpected command: ${file} ${args.join(" ")}`);
  ...
  const { id } = await dispatcher.dispatch({
    project: "fixture",
    prompt: "keep codex durable env",
    lane: "codex",
  });
  await waitForCondition(
    () => companionCommands.some(({ operation }) => operation === "status"),
    "Codex status polling did not start",
  );
  const durableEnv = launches[0].options.env;

  await assert.rejects(
    dispatcher.reply(id, {
      text: "accept cannot apply to live input",
      acceptExecutionProfile: true,
      actor: "human",
    }),
    (error) => error.status === 400 && /applies only to resumes/.test(error.message),
  );
  await assert.rejects(
    dispatcher.reply(id, { text: "Codex has no live input" }),
    (error) => error.status === 409 && /does not support live input/.test(error.message),
  );
  await dispatcher.stop(id);

  assert.ok(companionCommands.some(({ operation }) => operation === "cancel"));
  for (const { options } of companionCommands) {
    assert.equal(options.env, durableEnv);
    assert.equal(options.env.CLAUDE_PLUGIN_DATA, join(stateDir(), "codex-companion"));
  }
});

test("Codex stop keeps its fence when the pinned cancel executable cannot spawn", async (t) => {
  if (process.platform !== "linux") {
    t.skip("worker fencing uses Linux /proc identity");
    return;
  }
  ...
  _setRunFile(async (file, args) => {
    ...
    if (isCommand(file, "node") && args[1] === "status") {
      return JSON.stringify({ status: "running", job: { status: "running", pid: worker.child.pid } });
    }
    if (isCommand(file, "node") && args[1] === "cancel") {
      const error = new Error("spawn pinned node ENOENT");
      error.code = "ENOENT";
      error.path = file;
      throw error;
    }
  ...
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const { id } = await dispatcher.dispatch({ project: "fixture", prompt: "cancel", lane: "codex" });
  await waitForCondition(
    () => rawRecord(setup, id).codexWorkerPid === worker.child.pid,
    "Codex worker fence was not captured",
  );
...
test("Codex status ENOENT exhausts its retry budget and persists a profile failure", async (t) => {
  ...
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const { id } = await dispatcher.dispatch({ project: "fixture", prompt: "poll", lane: "codex" });
  await waitForConditionOverTime(
    () => dispatcher.get(id)?.state === "failed",
    "Codex status profile failure did not become terminal",
  );
  const failed = dispatcher.get(id);

  assert.equal(statusCalls, 3);
  assert.match(failed.exitSummary, /^EATELIER_EXECUTION_PROFILE_MISMATCH: /);
...
test("Codex provider identity stays profiled while verification uses workload env without plugin data", async (t) => {
  ...
  _setProbe(async () => ({ git: { dirtyCount: 0, branch: "main" } }));
  useLegacyCompanion(setup, () => companionPath);
  _setGitDirFileOps({ writeFileSync() {}, unlinkSync() {} });
...
test("codex lane stays warning-free when its resolved git dir is writable", async (t) => {
  ...
  assert.equal(record.executionProfile.executable.command, process.execPath);
  assert.equal(record.executionProfile.executable.resolvedPath, process.execPath);
  assert.equal(record.executionProfile.binaryPinned, false);
  assert.ok(record.warnings.some((warning) => warning.includes("cannot pin the codex binary")));
...
test("a failed Atelier-owned commit fails the dispatch before verification", async (t) => {
  ...
  assert.equal(
    ...
    "Atelier could not finalize completed Codex result [ERESULT_GIT]: Result finalization git command failed: git " +
      `-C ${record.worktreePath} commit -m chore(dispatch): finalize result [atelier-finalized] -- . :(exclude).beads :(exclude).atelier-workspace.json: identity unavailable`,
  );
});
...
test("merge fast-forwards the captured validated SHA with the exact argv cleanup sequence", async (t) => {
  ...
  assert.deepEqual(calls, [
    ...
    ["git", ["-C", setup.primary, "merge-base", "validated-head", "abcdef1234567890"]],
    [
      "git",
      [
        "-C",
...
test("legacy tokenless dispatch worktrees remain removable by merge and dismissal", async (t) => {
  const mergeSetup = await fixture(t);
  const mergeRecord = await seedDispatch(mergeSetup, { id: "legacy-merge" });
  await mkdir(mergeRecord.worktreePath, { recursive: true });
  _setRunFile(async (file, args) => {
    assert.equal(file, "git");
    if (args[2] === "worktree" && args[3] === "list") {
      return `worktree ${mergeSetup.primary}\n\nworktree ${mergeRecord.worktreePath}\n`;
    }
    if (args[2] === "worktree" && args[3] === "remove") {
      await rm(args[4], { recursive: true, force: true });
      return "";
  ...
  const merged = await mergeDispatcher.merge(mergeRecord.id);
  assert.equal(merged.merged.strategy, "ff");
  assert.equal(existsSync(mergeRecord.worktreePath), false);

  const dismissSetup = await fixture(t);
  const dismissRecord = await seedDispatch(dismissSetup, { id: "legacy-dismiss" });
  await mkdir(dismissRecord.worktreePath, { recursive: true });
  _setRunFile(async (file, args) => {
    assert.equal(file, "git");
    if (args[2] === "worktree" && args[3] === "list") {
      return `worktree ${dismissSetup.primary}\n\nworktree ${dismissRecord.worktreePath}\n`;
    }
    if (args[2] === "worktree" && args[3] === "remove") {
      await rm(args[4], { recursive: true, force: true });
      return "";
  ...
  const dismissed = await dismissDispatcher.dismiss(dismissRecord.id);
  assert.ok(dismissed.dismissed);
  assert.equal(existsSync(dismissRecord.worktreePath), false);
});
...
test("merge discards divergent dispatch .beads and preserves the committed main tracker tree", async (t) => {
  ...
  await writeFile(
    join(seeded.worktreePath, ".atelier-workspace.json"),
    `${JSON.stringify({
      recordId: seeded.id,
...
test("post-merge crash after worktree creation still cleans the detached worktree", async (t) => {
  ...
  _setRunFile(async (_file, args) => {
    ...
    if (args[2] === "worktree" && args[3] === "remove") {
      removedPath = args[4];
      postMergeIdentity = JSON.parse(readFileSync(
        join(args[4], ".atelier-workspace.json"),
        "utf8",
      ));
      return "";
  ...
  assert.deepEqual(
    {
      recordId: postMergeIdentity.recordId,
      project: postMergeIdentity.project,
      kind: postMergeIdentity.kind,
    },
    { recordId: seeded.id, project: "fixture", kind: "post-merge" },
  );
...
test("merge falls back to a clean checked-out primary branch", async (t) => {
  ...
  assert.deepEqual(calls, [
    ["git", ["-C", setup.primary, "rev-parse", "--verify", seeded.branch]],
    ["git", ["-C", setup.primary, "rev-parse", "main"]],
    ["git", ["-C", setup.primary, "merge-base", "validated-head", "1234567890abcdef"]],
    [
      "git",
      [
        "-C",
...
test("merge falls back to a detached worktree when primary is not on main", async (t) => {
  ...
  _setRunFile(async (file, args) => {
    calls.push([file, args]);
    if (args[2] === "worktree" && args[3] === "remove" && args[4].includes("merge-worktrees")) {
      mergeIdentity = JSON.parse(readFileSync(
        join(args[4], ".atelier-workspace.json"),
        "utf8",
      ));
    }
  ...
  assert.deepEqual(
    { recordId: mergeIdentity.recordId, project: mergeIdentity.project, kind: mergeIdentity.kind },
    { recordId: seeded.id, project: "fixture", kind: "merge" },
  );
  assert.deepEqual(calls, [
    ["git", ["-C", setup.primary, "rev-parse", "--verify", seeded.branch]],
    ["git", ["-C", setup.primary, "rev-parse", "main"]],
    ["git", ["-C", setup.primary, "merge-base", "validated-head", "feedface12345678"]],
    [
      "git",
      [
        "-C",
...
test("durable security events require the injected service writer", async (t) => {
  const setup = await fixture(t);
  const seeded = await seedDispatch(setup);
  stubFastForwardMerge(setup);
  ...
  await assert.rejects(
    dispatcher.mintBreakGlass(seeded.id, {
      action: "merge",
      targetSha: seeded.branchHead,
      ...FORCE_AUDIT,
      actor: "human-ui",
...
test("break-glass authorization is bound to result and attestation evidence at the same HEAD", async (t) => {
  for (const scenario of [
    {
      label: "result version",
      mutate(record) {
        record.result.version += 1;
        record.attestation.resultVersion += 1;
      },
      expected: /result evidence changed/,
    },
    {
      label: "attestation commit",
      mutate(record) {
        record.attestation.resultCommit = "re-attested-same-head";
      },
      expected: /attestation evidence changed/,
    },
  ]) {
    const setup = await fixture(t);
    const seeded = await seedDispatch(setup);
    ...
    const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state, eventLog });
    const authorization = await dispatcher.mintBreakGlass(seeded.id, {
      action: "merge",
      targetSha: seeded.branchHead,
      ...FORCE_AUDIT,
    ...
    const changed = rawRecord(setup, seeded.id);
...
test("break-glass authorization refuses expiry and a branch move after mint", async (t) => {
  ...
  const [expiredStoreName] = await readdir(join(expiredSetup.state, "break-glass"));
  const expiredStorePath = join(expiredSetup.state, "break-glass", expiredStoreName);
  ...
  await assert.rejects(
    ...
    /EATELIER_BREAK_GLASS_INVALID: authorization expired/,
    "clock rollback must not revive an expiry tombstone",
  );
...
test("boot recovers a consumed break-glass event debt from the durable token store", async (t) => {
  const setup = await fixture(t);
  const seeded = await seedDispatch(setup);
  stubFastForwardMerge(setup);
  const eventLog = createEventLog({ stateDir: setup.state });
  ...
  const failingLog = {
    append: (...args) => eventLog.append(...args),
    appendDurable(kind, payload, options) {
      if (kind === "dispatch.break-glass" && payload.phase === "consumed" && failConsumed) {
        failConsumed = false;
        throw new Error("fixture consumed audit EIO");
      }
      return eventLog.appendDurable(kind, payload, options);
    },
  };
  const dispatcher = createDispatcher({
    registry: setup.registry,
  ...
  await assert.rejects(
    ...
    /break-glass audit event could not be persisted/,
  );
  ...
  const consumedEvents = eventLog.read({ kind: "dispatch.break-glass" })
    .filter((event) => event.phase === "consumed");
  assert.equal(consumedEvents.length, 1);
  assert.equal(consumedEvents[0].recovered, true);
...
test("merge audit append debt never retroactively fails a landed merge and drains later and at boot", async (t) => {
  ...
  const failingLog = {
    append: (...args) => eventLog.append(...args),
    appendDurable(kind, payload, options) {
      if (kind === "dispatch.merge" && mergeFailures > 0) {
        mergeFailures -= 1;
        throw new Error("fixture merge audit EIO");
      }
      return eventLog.appendDurable(kind, payload, options);
    },
  };
  ...
  const merged = await forceMerge(dispatcher, seeded.id);
  assert.ok(merged.merged, "landed merge remains a successful operation");
  assert.equal(merged.postMerge.state, "skipped", "post-merge handoff still ran");
  assert.equal(merged.mergeEventDebt.attempts, 1);
  assert.ok(merged.warnings.some((warning) => warning.startsWith("merge audit event pending:")));

  const duplicate = await dispatcher.merge(seeded.id);
  assert.equal(duplicate.merged.commit, merged.merged.commit);
...
test("new break-glass mint supersedes the prior active token and doctor GC reaps old tombstones", async (t) => {
  ...
  const mint = () => dispatcher.mintBreakGlass(seeded.id, {
    action: "merge",
    targetSha: seeded.branchHead,
    forcedBy: "operator sk-abcdefghijklmnopq",
    reason: "token: hunter2-hunter2-hunter2",
    dispositionRef: "review/secret",
    actor: "human-ui",
  });
  ...
  await assert.rejects(
    dispatcher.merge(seeded.id, {
      force: true,
      breakGlassToken: first.token,
      actor: "human-ui",
    }),
    /authorization was superseded/,
  );
  const stores = (await readdir(join(setup.state, "break-glass"))).sort();
  assert.equal(stores.length, 2);
  const stored = await Promise.all(stores.map(async (name) =>
    JSON.parse(await readFile(join(setup.state, "break-glass", name), "utf8"))));
  assert.deepEqual(stored.map((entry) => entry.status).sort(), ["pending", "superseded"]);
  assert.doesNotMatch(JSON.stringify(stored), /sk-abcdefghijklmnopq|hunter2/);
  assert.deepEqual(
    eventLog.read({ kind: "dispatch.break-glass" }).map((event) => event.phase),
  ...
  await assert.rejects(
    dispatcher.merge(seeded.id, {
      force: true,
      breakGlassToken: second.token,
      actor: "human-ui",
    }),
    /authorization expired/,
  );
  const dryRun = await dispatcher.gc({
    dryRun: true,
    olderThanDays: 7,
    now: new Date(now + 8 * 24 * 60 * 60 * 1_000),
  });
  assert.equal(dryRun.breakGlassAuthorizations.length, 2);
  assert.equal((await readdir(join(setup.state, "break-glass"))).length, 2);
  const collected = await dispatcher.gc({
    olderThanDays: 7,
    now: new Date(now + 8 * 24 * 60 * 60 * 1_000),
  });
  assert.equal(collected.breakGlassAuthorizations.length, 2);
  assert.deepEqual(await readdir(join(setup.state, "break-glass")), []);
});
...
test("merge-commit validation binds result mode and object id before commit-tree", async (t) => {
  ...
  for (const scenario of scenarios) {
    ...
    await assert.rejects(dispatcher.merge(seeded.id), (error) => {
      ...
      assert.match(error.message, /main branch moved/);
      assert.match(error.message, /integrate main and re-verify, or use an audited force merge/);
      return true;
    });
...
test("reply reconciles a pending merge intent before it can add commits", async (t) => {
  const setup = await fixture(t);
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const intent = {
    resultCommit: "result-head",
  ...
  _setRunFile(async (_file, args) => {
    if (args[2] === "rev-parse" && args[3] === "main") {
      if (!recoveryAvailable) throw new Error("git temporarily unavailable");
      return "main-before\n";
    }
    if (args[2] === "merge-base") return "different-base\n";
    if (args[2] === "rev-parse" && args[3] === "-q") return "";
    return "";
  });

  await assert.rejects(
    dispatcher.reply(seeded.id, { text: "do not discard the merge" }),
    (error) => error.status === 409 && /merge intent is pending recovery/.test(error.message),
  );
  assert.deepEqual(rawRecord(setup, seeded.id).mergeIntent, intent);

  recoveryAvailable = true;
  const resumed = await dispatcher.reply(seeded.id, { text: "continue after reconciliation" });
  assert.equal(resumed.mergeRecoveryPending, false);
  assert.ok(["resuming", "running"].includes(resumed.state));
  assert.equal(rawRecord(setup, seeded.id).mergeIntent, null);
  await waitForState(dispatcher, seeded.id, ["completed", "completed_empty"]);
});

test("landed recovery resets interrupted primary state and leaves later merges usable", async (t) => {
  const setup = await fixture(t);
  const intent = {
    resultCommit: "result-head",
    branchHead: "result-head",
  ...
  const first = await seedDispatch(setup, { branchHead: "result-head", mergeIntent: intent });
  let phase = "recover";
  let secondMain = "landed-main";
  const calls = [];
  _setRunFile(async (_file, args) => {
    calls.push(args);
    if (phase === "recover") {
      ...
      if (args[2] === "merge-base") return "result-head\n";
      if (args[2] === "rev-parse" && args[3] === "-q") return "result-head\n";
      if (args[2] === "reset" && args[3] === "--hard") return "";
      if (args[2] === "rev-parse" && args[3] === "--verify") return "result-head\n";
      return "";
    }
    if (args[2] === "rev-parse" && args[3] === "--verify") return "validated-head\n";
    if (args[2] === "rev-parse" && args[3] === "main") return `${secondMain}\n`;
    if (args[2] === "merge-base") return "";
    if (args[2] === "fetch") {
      secondMain = "second-merged";
      return "";
    }
    return "";
  });
  ...
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  await waitForCondition(
    () => Boolean(dispatcher.get(first.id)?.merged),
    "restart did not recover the landed merge",
  );
  await settleAsyncWork();
  const recovered = dispatcher.get(first.id);
  assert.equal(recovered.merged.commit, "result-head");
  assert.ok(calls.some((args) =>
    args.join("\0") === ["-C", setup.primary, "reset", "--hard", "main"].join("\0")));

  phase = "second";
  const second = {
    ...rawRecord(setup, first.id),
    id: "dispatch-second",
    branch: "atelier/fixture-2-dispatch-second",
    worktreePath: join(setup.state, "worktrees", "fixture", "dispatch-second"),
    ticketId: "fixture-2",
    branchHead: "validated-head",
    result: { ...first.result, commit: "validated-head" },
    attestation: { resultCommit: "validated-head", resultVersion: 1 },
    merged: null,
    mergeIntent: null,
    mergeFollowUpDebt: undefined,
  };
  seededSyntheticBranches.add(second.branch);
  appendFileSync(
    join(setup.state, "dispatches", "index.jsonl"),
    `${JSON.stringify(second)}\n`,
  );
  const later = await dispatcher.merge(second.id);
  assert.equal(later.merged.commit, "second-merged");
});

test("already-contained intents clear as no-ops without fabricating unrelated landing commits", async (t) => {
  const setup = await fixture(t);
  const intent = {
    resultCommit: "result-head",
    branchHead: "result-head",
  ...
  _setRunFile(async (_file, args) => {
    calls.push(args);
    if (args[2] === "rev-parse" && args[3] === "main") return "unrelated-main-advance\n";
    if (args[2] === "rev-list") return "unrelated-commit\n";
    return "";
  });
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  ...
  await waitForCondition(
    () => rawRecord(setup, "dispatch-merge").mergeIntent === null,
    "already-contained intent did not clear",
  );
...
test("a cleanup failure during recovery becomes a warning, not a poisoned boot pass", async (t) => {
  ...
  await writeWorkspaceToken(seeded);
  _setRunFile(async (_file, args) => {
    if (args[2] === "rev-parse" && args[3] === "main") return "result-head\n";
    if (args[2] === "rev-parse" && args.includes("MERGE_HEAD")) throw new Error("no merge in progress");
    if (args[2] === "merge-base") return "result-head\n";
...
test("audited force abandons an unresolved intent and records exactly what it cleared", async (t) => {
  ...
  _setRunFile(async (_file, args) => {
    ...
    if (args[2] === "rev-parse" && args[3] === "main") return `${main}\n`;
    if (args[2] === "merge-base") return "";
    if (args[2] === "fetch") {
      main = "forced-merge";
      return "";
...
test("dismiss abandons a still-unresolved intent before removing the record artifacts", async (t) => {
  ...
  const dismissed = await dispatcher.dismiss("dispatch-merge");
  assert.ok(dismissed.dismissed);
  assert.equal(dismissed.mergeRecoveryPending, false);
  assert.ok(dismissed.warnings.includes(
    "merge intent abandoned by dismissal before recovery could resolve it",
  ));
  assert.equal(rawRecord(setup, "dispatch-merge").mergeIntent, null);
});

test("boot never resurrects dismissed intents and clears unreachable ones once", async (t) => {
  const reachableSetup = await fixture(t);
  const intent = {
    resultCommit: "result-head",
    branchHead: "result-head",
    mainBranch: "main",
  ...
  await seedDispatch(reachableSetup, {
    branchHead: "result-head",
    mergeIntent: intent,
    dismissed: { at: "2026-08-17T09:00:00.000Z" },
  });
  const reachableCalls = [];
  _setRunFile(async (_file, args) => {
    reachableCalls.push(args);
    if (args[2] === "for-each-ref") return "refs/heads/main\n";
    if (args[2] === "rev-parse" && args[3] === "main") return "landed-main\n";
    if (args[2] === "merge-base") return "result-head\n";
    return "";
  });
  const reachable = createDispatcher({
    registry: reachableSetup.registry,
    stateDir: reachableSetup.state,
  });
  await settleAsyncWork();
  assert.ok(reachable.get("dispatch-merge").dismissed);
  assert.equal(reachable.get("dispatch-merge").merged, null);
  assert.equal(reachableCalls.some((args) => args[2] === "merge-base"), false);

  const unreachableSetup = await fixture(t);
  await seedDispatch(unreachableSetup, {
    branchHead: "result-head",
    mergeIntent: intent,
    dismissed: { at: "2026-08-17T09:00:00.000Z" },
  });
  ...
  const unreachable = createDispatcher({
    registry: unreachableSetup.registry,
    stateDir: unreachableSetup.state,
  });
  await waitForCondition(
    () => rawRecord(unreachableSetup, "dispatch-merge").mergeIntent === null,
    "unreachable dismissed intent did not clear",
  );
  assert.ok(unreachable.get("dispatch-merge").warnings.some((warning) =>
    /cleared because commit result-head is unreachable/.test(warning)));
  createDispatcher({ registry: unreachableSetup.registry, stateDir: unreachableSetup.state });
  await settleAsyncWork();
  assert.equal(reachabilityChecks, 1);
});

test("recovered merges clean artifacts with the live branch-ref recheck", async (t) => {
  for (const advanced of [false, true]) {
    ...
    const seeded = await seedDispatch(setup, { branchHead: "result-head", mergeIntent: intent });
    const calls = [];
    _setRunFile(async (_file, args) => {
      calls.push(args);
      if (args[2] === "rev-parse" && args[3] === "main") return "result-head\n";
    ...
    assert.equal(
      calls.some((args) => args[2] === "branch" && args[3] === "-D"),
      !advanced,
    );
    if (advanced) {
      assert.ok(recovered.warnings.includes(
        `branch cleanup skipped: ${seeded.branch} advanced from merged result-head to advanced-head`,
      ));
    }
  }
});

test("inline recovered convoy merge starts the next member without a restart", async (t) => {
  const setup = await fixture(t, { tracker: "committed" });
  const issues = [
  ...
  _setSpawner(() => successfulChild());
  const firstDispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const convoy = await firstDispatcher.createConvoy("fixture", {
    ticketIds: ["fixture-1", "fixture-2"],
  });
  const first = await waitForState(firstDispatcher, convoy.currentDispatchId, ["completed"]);
  const rawFirst = rawRecord(setup, first.id);
  const intent = {
    resultCommit: rawFirst.result.commit,
    branchHead: rawFirst.result.commit,
    mainBranch: "main",
    mainTipBefore: "main-before",
    alreadyContained: false,
    startedAt: "2026-08-17T08:00:00.000Z",
  ...
  appendFileSync(
    join(setup.state, "dispatches", "index.jsonl"),
    `${JSON.stringify({ ...rawFirst, mergeIntent: intent })}\n`,
  );

  let mainReads = 0;
  stubConvoyRuntime(setup, issues, [], {
    gitHandler: async (args) => {
      if (args[2] === "rev-parse" && args[3] === "main") {
        mainReads += 1;
        if (mainReads === 1) throw new Error("boot git unavailable");
        return "landed-main\n";
      }
      if (args[2] === "merge-base") return `${rawFirst.result.commit}\n`;
      if (args[2] === "rev-list") return "landing-commit\n";
      if (args[2] === "rev-parse" && args[3] === "-q") return "";
      return undefined;
    },
  });
  _setSpawner(() => successfulChild());
  const successor = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  await waitForCondition(() => mainReads === 1, "boot recovery did not attempt reconciliation");

  const recovered = await successor.merge(first.id);
  assert.equal(recovered.merged.commit, "landing-commit");
  const advanced = await waitForConvoy(
    successor,
    convoy.id,
    (candidate) => candidate.cursor === 1 && Boolean(candidate.currentDispatchId),
  );
  assert.equal(successor.get(advanced.currentDispatchId).ticketId, "fixture-2");
  await waitForState(successor, advanced.currentDispatchId, ["completed"]);
});
...
test("concurrent merge and reply reserve exactly one lifecycle winner", async (t) => {
  ...
  await writeWorkspaceToken(mergeTarget);
  const mergeCalls = [];
  ...
  await writeWorkspaceToken(replyTarget);
  const replyCalls = [];
...
test("concurrent merge and dismiss reserve exactly one lifecycle winner", async (t) => {
  ...
  await writeWorkspaceToken(dismissTarget);
  const dismissCalls = [];
...
test("dismissal refuses a workspace whose identity token mismatches the record", async (t) => {
  ...
  const seeded = await seedDispatch(setup);
  await mkdir(seeded.worktreePath, { recursive: true });
  await writeWorkspaceToken({ ...seeded, id: "different-dispatch" });
  const calls = [];
  _setRunFile(async (file, args) => {
    calls.push([file, args]);
    return "";
  ...
  await assert.rejects(
    dispatcher.dismiss(seeded.id),
    (error) => error.status === 409 && /identity token mismatch/.test(error.message),
  );
  assert.equal(existsSync(seeded.worktreePath), true);
  assert.equal(calls.some(([, args]) =>
    args[2] === "worktree" && args[3] === "remove" && args[4] === seeded.worktreePath), false);
  assert.ok(dispatcher.get(seeded.id).warnings.some((warning) =>
    /identity token mismatch/.test(warning)));
});

test("gc refuses a symlinked orphan candidate that escapes its owning root", async (t) => {
  const setup = await fixture(t);
  const projectRoot = join(setup.state, "worktrees", "fixture");
  const outside = join(setup.root, "outside-worktree");
  const candidate = join(projectRoot, "symlink-orphan");
  ...
  assert.deepEqual(result.orphans, []);
  assert.ok(result.warnings.some((warning) =>
    warning.includes(candidate) && /not a real directory|escapes/.test(warning)));
  assert.equal(await readFile(join(outside, "marker"), "utf8"), "keep\n");
  assert.equal(calls.some(([, args]) => args.includes(candidate) && args.includes("remove")), false);
});

test("gc aborts when an authorized orphan directory is replaced before removal", async (t) => {
  const setup = await fixture(t);
  const projectRoot = join(setup.state, "worktrees", "fixture");
  const candidate = join(projectRoot, "replace-before-remove");
  const displaced = `${candidate}-original`;
  await mkdir(candidate, { recursive: true });
  await writeWorkspaceToken({
    id: "replace-before-remove",
    project: "fixture",
    startedAt: "2026-07-01T00:00:00.000Z",
  }, { path: candidate, kind: "dispatch" });
  const dirent = (name) => ({ name, isDirectory: () => true, isSymbolicLink: () => false });
  let candidateLstats = 0;
  let removals = 0;
  _setGcFileOps({
    readdirSync(path) {
      if (path === join(setup.state, "worktrees")) return [dirent("fixture")];
      if (path === projectRoot) return [dirent("replace-before-remove")];
      return [];
    },
    lstatSync(path) {
      if (path === candidate) {
        candidateLstats += 1;
        if (candidateLstats === 2) {
  ...
  _setRunFile(async (_file, args) =>
    args[2] === "worktree" && args[3] === "list" ? `worktree ${setup.primary}\n` : "");
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  ...
  assert.ok(result.warnings.some((warning) =>
    warning.includes(candidate) && /changed before removal/.test(warning)));
});
...
test("convoy pause persistence failure keeps the failed transition and surfaces warnings", async (t) => {
  ...
  _setPersistenceFileOps({
    appendFileSync,
    writeDescriptorSync(descriptor, contents, encoding, path) {
      if (
        !failedPauseWrite &&
        isAtomicWriteFor(path, "convoys.json") &&
...
test("ready queue resume rolls back when queue persistence fails", async (t) => {
  ...
  _setPersistenceFileOps({
    appendFileSync,
    writeDescriptorSync(descriptor, contents, encoding, path) {
      if (isAtomicWriteFor(path, "queue.json")) {
...
test("dispatcher boots with intact records when oversized-index compaction hits ENOSPC", async (t) => {
  ...
  _setPersistenceFileOps({
    appendFileSync,
    writeDescriptorSync(descriptor, contents, options, path) {
      if (isAtomicWriteFor(path, "index.jsonl")) {
  ...
  assert.equal((await readdir(dispatchDir)).some((name) => name.endsWith(".tmp")), false);
});
...
test("rollup de-duplicates persisted history and sums projects and local days", async (t) => {
  ...
  assert.deepEqual(dispatcher.rollup(), {
    ...
    days: [
      { day: localDay(1), runs: 2, costUSD: 1.25 },
      { day: localDay(0), runs: 2, costUSD: 2.75 },
    ],
    totals: { runs: 4, merged: 1, forcedMerged: 1, turns: 8, costUSD: 4 },
  });
});
...
test("boot refuses codex reattach when companion resolution changed", async (t) => {
  ...
  await writeFile(join(dispatchDir, "index.jsonl"), `${JSON.stringify(record)}\n`);
  useLegacyCompanion(setup, () => newCompanion);
...
test("restoring the recorded Codex companion clears a boot refusal before reply", async (t) => {
  ...
  const oldCompanionSource = "// restored fixture companion\n";
  await writeFile(oldCompanion, oldCompanionSource);
  ...
  const executionProfile = createExecutionProfile({
    ...
    executableResolvedPath: process.execPath,
    companionPath: oldCompanion,
    companionDigest: createHash("sha256").update(oldCompanionSource).digest("hex"),
    binaryPinned: false,
    env: profileEnv,
    controlledKeys: ["ATELIER_PRIMARY_CHECKOUT", "ATELIER_TRACKER_PATH"],
    hooksSupported: true,
  });
  await rm(oldCompanion);
  const record = {
  ...
  await writeFile(oldCompanion, oldCompanionSource);
  useLegacyCompanion(setup, () => oldCompanion);
  const launches = [];
  _setSpawner((command, args, options) => {
    launches.push({ command, args, options });
...
test("integration: a real git worktree is added for a dispatch", async (t) => {
  ...
  const excludePath = join(setup.primary, ".git", "info", "exclude");
...
test("boot recovers a missing automatic review exactly once after the completion handoff", async (t) => {
  ...
  await mkdir(target.worktreePath, { recursive: true });
  await writeWorkspaceToken(target);
  _setProbe(async () => ({ git: { dirtyCount: 0, branch: "main" } }));
  _setRunFile(async (_file, args) => {
    if (isOutcomeDiffProbe(args)) return PROBE_CHANGED_FILE;
    if (args[2] === "diff") {
...
test("requireReview preserves flat and mid-upgrade passes for merge and never merges review dispatches", async (t) => {
  ...
  await assert.rejects(
    forceMerge(reviewDispatcher, reviewRecord.id),
...
test("verification re-run reconciles profiles in memory and after boot, with explicit acceptance", async (t) => {
  const setup = await fixture(t, {
    verifyCommands: ["node profile-rerun"],
    dispatchEnv: { PROFILE_RERUN_FLAG: "before" },
  });
  stubPreparation();
  let verifyRuns = 0;
  _setSpawner((command) => {
    if (isCommand(command, "claude")) return successfulChild();
    verifyRuns += 1;
    return verifyChild({ code: verifyRuns === 1 ? 1 : 0 });
  });
  const dispatcher = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const { id } = await dispatcher.dispatch({ project: "fixture", prompt: "profile rerun" });
  const failedVerify = await waitForState(dispatcher, id, ["completed"]);
  assert.equal(failedVerify.verify.state, "failed");
  const originalProfile = structuredClone(failedVerify.executionProfile);

  setup.project.dispatchEnv = { PROFILE_RERUN_FLAG: "after" };
  await assert.rejects(
    dispatcher.rerunVerification(id),
    (error) => {
      assert.equal(error.status, 409);
      assert.match(error.message, /^EATELIER_EXECUTION_PROFILE_MISMATCH: /);
      assert.match(error.message, /PROFILE_RERUN_FLAG/);
      return true;
    },
  );
  assert.equal(verifyRuns, 1);
  assert.equal(dispatcher.get(id).attestation, null);
  assert.deepEqual(dispatcher.get(id).executionProfile, originalProfile);
  await dispatcher.shutdown({ graceMs: 0 });

  const successor = createDispatcher({ registry: setup.registry, stateDir: setup.state });
  const admitted = await successor.rerunVerification(id, {
    acceptExecutionProfile: true,
    actor: "human",
  });
  assert.equal(admitted.state, "verifying");
  assert.equal(admitted.executionProfile.superseded.length, 1);
  assert.equal(admitted.executionProfile.superseded[0].reason, "operator accepted");
  assert.equal(admitted.executionProfile.superseded[0].actor, "human");
...
test("integration: a flaky verification is recoverable - the re-run passes, both attempts survive, and merge needs no force", async (t) => {
  ...
  setup.project.dispatchEnv = { POST_MERGE_PROFILE_DRIFT: "advisory-only" };
  ...
  const postMergeRecord = dispatcher.get(id);
  assert.ok(postMergeRecord.warnings.some((warning) =>
    warning.startsWith("post-merge verification environment diverged: EATELIER_EXECUTION_PROFILE_MISMATCH: ") &&
    warning.includes("POST_MERGE_PROFILE_DRIFT")));
  assert.ok(rawRecord(setup, id).warnings.some((warning) =>
    warning.includes("POST_MERGE_PROFILE_DRIFT")));
});
...
async function seedCodexRecord(setup, overrides = {}) {
  ...
  await writeFile(join(dispatchDir, "index.jsonl"), `${JSON.stringify(record)}\n`);
  if (existsSync(worktreePath)) await writeWorkspaceToken(record);
  return record;
}
...
test("a terminal record whose process tree was reaped still resumes - the reap costs a cold start, not the session", async (t) => {
  ...
  const orphanWarnings = record.warnings.filter((warning) => warning.startsWith("Codex orphan process "));
  assert.equal(orphanWarnings.length, 3);
  for (const warning of orphanWarnings) {
    assert.match(warning, /left alone: dispatch codex-leak is completed and its worktree is still present$/);
  }
  assert.deepEqual(record.warnings.filter((warning) => !warning.startsWith("Codex orphan process ")), [
...
test("gc retains a failed post-merge worktree while its runner is unconfirmed, then removes it after proof of death", async (t) => {
  ...
  await writeWorkspaceToken(incident, { path: worktreePath, kind: "post-merge" });
  _setRunFile(async () => "");
  const dispatcher = createDispatcher({
    registry: setup.registry,
    stateDir: setup.state,
